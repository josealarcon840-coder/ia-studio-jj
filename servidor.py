...
MODELS = [
    ...
    # 🔴 TEXTURAS MÁGICAS ACTUALIZADAS (Parche de vuelta y control estricto de colores y fondos)
    {"slug": "textile-styles", "label": L("Texturas Mágicas 3D", "3D Magic Textures"), "icon": "fa-cubes", "category": L("5. Belleza y Edición", "5. Beauty & Edit"), "desc": L("Aplica lana, bordado, parche o globo.", "Applies yarn, thread, patch or puffy textures."), "endpoint": "/v1/images/edits", "response_type": "image", "fields": [
        ...
        {"name": "prompt", "type": "select", "label": L("Elige la Textura", "Select Texture"), "required": True, "options": [
            {"value": "Apply 3D amigurumi crochet texture. CRITICAL INSTRUCTION: You MUST keep the EXACT original colors and palette. If the image has a background, convert the background to crochet too. If the background is transparent, strictly PRESERVE the transparency and only texture the visible design. Change surfaces to knitted yarn matching original colors.", "label": L("🧶 Crochet / Amigurumi (Lana)", "Crochet / Amigurumi")},
            {"value": "Apply highly detailed realistic embroidery texture. CRITICAL INSTRUCTION: You MUST use threads that EXACTLY match the original colors. DO NOT invent colors. If there is a background, embroider the background too. If the background is transparent, strictly PRESERVE the transparency. Change surfaces to stitched thread.", "label": L("🧵 Bordado Realista (Hilos 3D)", "Realistic Embroidery")},
            {"value": "Apply textile embroidery patch style. CRITICAL INSTRUCTION: Keep EXACT original colors. If the image has a background, turn the entire background into a patched fabric style. If the background is transparent, strictly PRESERVE the transparency. Do not add white backgrounds.", "label": L("🏷️ Parche Textil", "Textile Patch")},
            {"value": "Apply 3D inflated balloon puffy texture. CRITICAL INSTRUCTION: Keep EXACT original colors. If there is a background, inflate the background too. If the background is transparent, strictly PRESERVE the transparency. Make elements look like soft, glossy plastic or vinyl.", "label": L("🎈 Estilo Inflado 3D (Globo/Puffer)", "3D Inflated/Puffer")}
        ]}
    ]},
